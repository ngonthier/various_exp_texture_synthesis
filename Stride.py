...
def loss_n_stats(sess,net,image_style,M_dict,n,TypeOfComputation='moments'):
    ...
    for layer, weight in style_layers:
        ...
        if(TypeOfComputation=='moments'):
            ...
            weight_help_convergence *= 10**(-9) ### Be careful because nan otherwise for n = 4 et pour les couches de Gatys
            print("Need to reduce the value of weight_help_convergence")
...
def loss_variance(sess,net,image_style,M_dict):
    ...
    for layer, weight in style_layers:
        ...
        #print(sig_x.get_shape())
        print(variance_x.get_shape())
        #mean_a = tf.reduce_mean(x,axis=axis)
        #variance_a = tf.subtract(tf.reduce_mean(tf.pow(a,2), axis=axis),mean_a)
        #sig_a =  tf.sqrt(variance_a)
        #style_loss = tf.nn.l2_loss(tf.subtract(sig_x,sig_a))
...
def loss__HF_filter(sess, net, image_style,M_dict):
    ...
    M = M_dict[layer[:5]]
    n = 4
    M_x = compute_n_moments(conv_x,n)
    M_a = compute_n_moments(conv_a,n)
    style_loss = tf.nn.l2_loss(tf.subtract(M_x,M_a))
    
    #G_x = gram_matrix(conv_x,N,M)
    #G_a = gram_matrix(conv_a,N,M)
    #style_loss = tf.nn.l2_loss(tf.subtract(G_x,G_a))
    
    #P_a = tf.nn.l2_loss(conv_a)
    #P_x = tf.nn.l2_loss(conv_x)
    #style_loss = tf.abs(P_a-P_x)
